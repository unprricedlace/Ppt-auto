...
def generate_commentary(data, period, at_par_threshold):
    ...
    # Check salaries impact
    salaries_variance = data.loc[data['Expense Details'] == 'Salaries', variance_col].values[0]
    if not pd.isna(salaries_variance) and abs(salaries_variance) > 0:
        salaries_amount = format_currency(abs(salaries_variance))
        if salaries_variance < 0:
            if employees_variance < 0:  # Lower headcount
                comp_favorable_drivers.append(f"lower salaries ({salaries_amount}) driven by lower headcount ({int(employees)})")
            else:
                comp_favorable_drivers.append(f"lower salaries ({salaries_amount})")
        else:
            if employees_variance > 0:  # Higher headcount
                comp_unfavorable_drivers.append(f"higher salaries ({salaries_amount}) driven by higher headcount ({int(employees)})")
            else:
                comp_unfavorable_drivers.append(f"higher salaries ({salaries_amount})")
    
    # Check employee benefits impact
    benefits_variance = data.loc[data['Expense Details'] == 'Employee Benefits', variance_col].values[0]
    if not pd.isna(benefits_variance) and abs(benefits_variance) > 0:
        benefits_amount = format_currency(abs(benefits_variance))
        if benefits_variance < 0:
            comp_favorable_drivers.append(f"lower employee benefits ({benefits_amount})")
        else:
            comp_unfavorable_drivers.append(f"higher employee benefits ({benefits_amount})")
    ...
    if not pd.isna(pos_variance) and abs(pos_variance) > 0:
        ...
        if pos_variance > 0:
            noncomp_unfavorable_drivers.append(f"higher Professional &O/S ({pos_amount})")
            noncomp_drivers_direction.append(-1)
        else:
            noncomp_favorable_drivers.append(f"lower Professional &O/S ({pos_amount})")
            noncomp_drivers_direction.append(1)
        
        # Check if contractors variance is significant (more than 40%)
        if abs(contractors_variance_pct) > 40:
            contractors_text = f"{'higher' if contractors_variance > 0 else 'lower'} contractors than budget"
            ...
            if (contractors_variance > 0 and pos_variance > 0) or (contractors_variance < 0 and pos_variance < 0):
                ...
                if pos_variance > 0:
                    noncomp_unfavorable_drivers[-1] += f" due to {contractors_text}"
                else:
                    noncomp_favorable_drivers[-1] += f" due to {contractors_text}"
            else:
                # If they're in opposite directions, note as an offset
                if pos_variance > 0:
                    noncomp_unfavorable_drivers[-1] += f", partially offset by {contractors_text}"
                else:
                    noncomp_favorable_drivers[-1] += f", partially offset by {contractors_text}"
